...
1. **Только своя секция** — одним сообщением, без `/paste_transcript`:
   ...
   ...
   ```
   Бот сразу разберёт @-секцию, отправит задачи участнику и **покажет ведущему список**.

2. **Весь «План действий» одним сообщением** — `/paste_transcript`, затем текст с
   одной или несколькими @-секциями → сразу обработка и отчёт ведущему.

3. **По частям** — `/paste_transcript` → `@Speaker 1 …` → `@Степан …` → …
   Каждая полная @-секция обрабатывается сразу; `/paste_done` — для хвоста без
   распознанных @-заголовков.

**Повторная вставка** после рассылки: бот спросит «Разослать заново» или «Только сохранить».

**Отчёт ведущему** после обработки: кому отправлен экран подтверждения **и какие
задачи**, у кого нет задач в транскрипте, кто не сопоставлен (@ → кнопки назначения).
