...
(ручной и auto из Plaud), чек-ин, декомпозиция, голос, итоги и Sheets, `/my_goals_stats`,
...
| `/my_goals_stats` | Прогресс по неделям (серия, % выполнения, частые затыки) |
| `/my_goals_submit` | Обновить задачи в таблице (вкладка «Прогресс») |
| `/my_profile` | Посмотреть JTBD-профиль (анкета) |
| `/my_profile_fill` | Заполнить или обновить анкету про бизнес и цели |
...
1. **Только своя секция** — одним сообщением, без `/paste_transcript`:
   ...
   ...
   ```
   Бот сохранит текст, покажет ведущему **превью** задач и кнопки
   «Разослать участникам» / «Только сохранить». Участнику экран уйдёт только
   после «Разослать».

2. **Весь «План действий» одним сообщением** — `/paste_transcript`, затем текст с
   одной или несколькими @-секциями → то же превью и подтверждение ведущего.

3. **По частям** — `/paste_transcript` → `@Speaker 1 …` → `@Степан …` → …
   Каждая полная @-секция сразу в превью; `/paste_done` — для хвоста без
   распознанных @-заголовков.

Пиши `@Имя Фамилия` (с пробелом): иначе Telegram может показать чужой публичный
`@username` (это превью клиента, не бот).

**Повторная вставка** после рассылки: в превью будет пометка, что участникам уже
отправляли — «Разослать» обновит экраны.

**Отчёт ведущему** после «Разослать»: кому ушёл экран подтверждения и какие задачи;
несопоставленные @ → кнопки назначения. В превью — «Править задачи» (выбрать
участника, прислать новый список). То же из `/group` → Задачи группы →
«Править задачи участника».
