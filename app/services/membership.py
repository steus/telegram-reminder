...
def validate_member_name_latin(name: str) -> bool:
    """Имя для Plaud: латиница, без кириллицы, 2–100 символов."""
...
def format_members_list(
    *,
    ...
    """members: (id, full_name, chat_id, is_active)."""
    if not members:
        return "В группе пока нет активных участников."

    lines = ["Участники группы:", ""]
    for member_id, full_name, chat_id, is_active in members:
        badges: list[str] = []
