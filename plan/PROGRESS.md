...
- **Этап 8A (в работе):** `member_profile` + `OnboardingStatus`, state
  ...
  **Фикс:** ведущий в `onboarding_survey` — `@`-секции/`group_paste_*` уходили
  ...
  Перед рассылкой — превью + кнопки «Разослать» / «Только сохранить»
  (`preview_paste_assignments`); участнику экран только после кнопки.
  Ведущий может править задачи участника: кнопка «Править задачи» в превью
  и «Править задачи участника» в меню `/group` → Задачи.
  Plaud/вставка плана — в том же подменю «Задачи группы» (не отдельный раздел).
